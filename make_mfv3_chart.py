...
v2 = equity("backtest_mfv2_trades_60s.csv")
v3 = equity("backtest_mfv3__trades_60s.csv")
allv = v2 + v3
...
svg = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" font-family="sans-serif">',
       ...
       f'<text x="{W//2}" y="20" fill="#e8e8e8" font-size="15" text-anchor="middle">MaxFade v2 বনাম v3 — ইক্যুইটি ($1, পেআউট 85%)</text>']
...
svg.append(f'<text x="{PL+10}" y="{H-12}" fill="#7aa2f7" font-size="12">━ v2: 10,574 ট্রেড, 59.62%, +$1,043</text>')
svg.append(f'<text x="{PL+380}" y="{H-12}" fill="#9ece6a" font-size="12">━ v3: 10,772 ট্রেড, 59.66%, +$1,070</text>')
svg.append("</svg>")
open("chart_mfv3.svg", "w").write("\n".join(svg))
print("wrote chart_mfv3.svg")
