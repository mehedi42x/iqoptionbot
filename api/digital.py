"""
IQ Option Digital Trading & Market Data API Module
Responsible for Digital option candle retrieval, trade execution, and trade resolution.
...
logger = logging.getLogger("IQDigitalAPI")
...
class DigitalAPI:
    ...
    def get_candles(self, symbol: str, timeframe: int, count: int) -> List[Dict[str, Any]]:
        """
        Fetch candles for digital instruments.
        """
        if self.auth._is_mock or not self.auth.api:
            ...
            for i in range(count):
                ...
                open_p = base_price + (i * 0.04) - (0.015 * (i % 3))
                close_p = open_p + 0.025 if i % 2 == 0 else open_p - 0.015
        ...
        try:
            ...
            logger.error(f"Error fetching digital candles: {e}")
            return []
    ...
    def execute_trade(self, symbol: str, amount: float, action: str, duration: int) -> Tuple[bool, Optional[str], Optional[Dict[str, Any]]]:
        ...
        if self.auth._is_mock or not self.auth.api:
            trade_id = f"mock_dig_{int(time.time())}"
            info = {
                "id": trade_id,
                "symbol": symbol,
                "amount": amount,
        ...
        try:
            check, order_id = self.auth.api.buy_digital_spot(symbol, amount, action_clean, duration)
            if check:
                info = {
                    "id": str(order_id),
                    "symbol": symbol,
                ...
                return False, f"Digital trade rejected: {order_id}", None
        except Exception as e:
            return False, f"Digital execution error: {str(e)}", None

    def check_trade_result(self, trade_id: str) -> Tuple[str, float]:
        """
        Check digital option trade resolution.
        """
        if self.auth._is_mock or not self.auth.api:
            return "WIN", 8.8

        try:
            check, pnl = self.auth.api.check_win_digital_v2(trade_id)
            if check:
            ...
            logger.error(f"Error checking digital trade result: {e}")
            return "PENDING", 0.0
