#!/usr/bin/env python3
"""V1 vs V2 vs MaxFade equity @85% payout, $1 stake (stdlib only)."""
import csv

PAYOUT = 0.85
SERIES = [
    ("backtest_v1_85_trades_60s.csv", "V1 emacombo", "#dc2626"),
    ("backtest_v2_85_trades_60s.csv", "V2 emacombo_v2", "#f59e0b"),
    ("backtest_mf_full_trades_60s.csv", "MaxFade FULL (21.8k)", "#3b82f6"),
    ("backtest_mf_trades_60s.csv", "MaxFade TRIM (16.1k)", "#16a34a"),
]
...
def main():
    ...
    for (label, color, _), e in zip(curves, eqs):
        paths.append(f'<path d="M' + " L".join(f"{X(i):.1f},{Y(v):.1f}" for i, v in enumerate(e))
    ...
    for (label, color, _), e in zip(curves, eqs):
        ...
        lx += 250 if "MaxFade" in label else 210

    svg = f'''<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" font-family="sans-serif">
<rect width="{W}" height="{H}" fill="white"/>
<text x="{W//2}" y="20" font-size="14" font-weight="bold" text-anchor="middle" fill="#111827">V1 vs V2 vs MaxFade — 60s expiry @85% payout, $1 stake</text>
{''.join(grid)}
...
<g font-size="12" fill="#111827">{''.join(legend)}</g>
    ...
    open("chart_mf.svg", "w").write(svg)
    print("wrote chart_mf.svg")
