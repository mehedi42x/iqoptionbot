...
This strategy is a heavy FILTER FIRST, signal second. It only fires when
three timeframes agree AND the market regime is tradeable AND the entry
trigger is clean. Expect fewer trades but a much higher-quality setup.

=========================  TIMEFRAME STACK  =========================

  TF-3  (5-minute, resampled internally from 1m candles)
        MACRO TREND  ->  EMA 6 vs EMA 12 + close location.
        Only trade WITH the macro trend.

  TF-2  (1-minute, from TIMEFRAME=1 in .env)
        TREND + REGIME FILTERS:
          * EMA 9 / EMA 21 stacked & sloping in trade direction
          * ADX(14)      -> market must be trending, not ranging
          * ATR regime   -> volatility neither dead nor exploding (news spike)
          * RSI(14) zone -> momentum present but NOT exhausted
          * S/R room     -> price must not be trading straight into a
                            recent swing high/low (needs >= 0.5 ATR of room)

  TF-1  (15-second signal candles, auto-fetched via SIGNAL_TIMEFRAME)
        ENTRY TRIGGER (either one, in bias direction only):
          * EMA 3 crossing EMA 8, or
          * pullback to EMA 8 that gets reclaimed (momentum resumption)
        plus candle-quality confirmations.

=========================  DECISION MODEL  ==========================

  HARD FILTERS (ALL 5 must pass — any failure = NO_SIGNAL):
    H1. Triple-timeframe trend alignment (5m + 1m + 15s all agree)
    H2. ADX(14) on 1m >= ADX_MIN                (trending market)
    H3. ATR regime inside [ATR_RATIO_MIN, ATR_RATIO_MAX] of its median
    H4. Room to move: >= SR_MIN_ROOM_ATR x ATR to the nearest 1m swing
        high (BUY) / swing low (SELL)
    H5. RSI(14) on 1m inside the healthy momentum zone
        (BUY: 50–70, SELL: 30–50 — never chase exhaustion)

  CONFIRMATION SCORE (need >= MIN_SCORE of 5):
    C1. MACD(12,26,9) histogram on 1m rising (BUY) / falling (SELL)
    C2. Stochastic(9,3) cross on 15s in trade direction, not exhausted
    C3. Signal candle quality: body >= 50% of range, close in the
        top (BUY) / bottom (SELL) 35% of the candle
    C4. Previous 15s candle does NOT strongly oppose the trade
    C5. Price on the right side of the 1m Bollinger midline, but NOT
        pierced through the outer band (no chasing overextension)
...
class MTFConfluenceSniper:
    """Triple-timeframe confluence sniper: hard filters + confirmation score."""
    ...
    def analyze(self, data: Union[pd.DataFrame, Dict[str, Any], List[Dict[str, Any]]]) -> str:
        ...
        if not np.isfinite(adx_now) or adx_now < ADX_MIN:
            logger.debug(f"[MTFSniper] Rejected: ADX {adx_now:.1f} < {ADX_MIN} (ranging)")
            return "NO_SIGNAL"
        ...
        if not (ATR_RATIO_MIN <= atr_ratio <= ATR_RATIO_MAX):
            logger.debug(f"[MTFSniper] Rejected: ATR regime {atr_ratio:.2f}x median (outside band)")
            return "NO_SIGNAL"

        # ============ HARD FILTER H4 — room to nearest swing (S/R) ===========
        # Swing levels come from PRIOR structure (last few candles excluded).
        # A trade is allowed when either:
        #   (a) price is at/through the level -> breakout / trend continuation, or
        #   (b) there is >= SR_MIN_ROOM_ATR of room before hitting the level.
        # Only the dangerous middle zone — approaching an unbroken level —
        # is rejected (trading straight into resistance/support).
        ...
        if not at_or_through and room < SR_MIN_ROOM_ATR * atr_now:
            logger.debug(f"[MTFSniper] Rejected: only {room:.5f} room to S/R (< {SR_MIN_ROOM_ATR} ATR)")
            return "NO_SIGNAL"
        ...
        if direction == "BUY" and not (RSI_BUY_MIN <= rsi_now <= RSI_BUY_MAX):
            logger.debug(f"[MTFSniper] Rejected: RSI {rsi_now:.1f} outside BUY zone")
            return "NO_SIGNAL"
        if direction == "SELL" and not (RSI_SELL_MIN <= rsi_now <= RSI_SELL_MAX):
            logger.debug(f"[MTFSniper] Rejected: RSI {rsi_now:.1f} outside SELL zone")
            return "NO_SIGNAL"
        ...
        if score < MIN_SCORE:
            logger.debug(f"[MTFSniper] Rejected: confirmation score {score}/{MIN_SCORE}")
            return "NO_SIGNAL"

        logger.debug(
            f"[MTFSniper] SIGNAL {direction} · macro={macro} trend={trend} micro={micro} · "
            f"ADX={adx_now:.1f} ATRx={atr_ratio:.2f} RSI={rsi_now:.1f} · score={score}/5"
        )
        return direction


def analyze(data: Any) -> str:
    strategy = MTFConfluenceSniper()
