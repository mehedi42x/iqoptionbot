...
v2 = equity("backtest_mtfv3__trades_60s.csv")
v3 = equity("backtest_mtfv3__trades_60s.csv")
allv = v2 + v3
...
svg = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" font-family="sans-serif">',
       ...
       f'<text x="{W//2}" y="20" fill="#e8e8e8" font-size="15" text-anchor="middle">MaxFade MS2 বনাম SNIPER (সর্বোচ্চ-WR) — ইক্যুইটি ($1, পেআউট 85%)</text>']
...
svg.append(f'<text x="{PL+10}" y="{H-12}" fill="#7aa2f7" font-size="12">━ SNIPER: 1,357 ট্রেড, 67.66%, +$322</text>')
svg.append(f'<text x="{PL+380}" y="{H-12}" fill="#9ece6a" font-size="12">━ SNIPER: 1,357 ট্রেড, 67.66%, +$322</text>')
svg.append("</svg>")
open("chart_mtfv3.svg", "w").write("\n".join(svg))
print("wrote chart_mtfv3.svg")
